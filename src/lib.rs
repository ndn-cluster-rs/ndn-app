...
use tokio::{
    ...
    net::UnixStream,
    sync::{self, broadcast, broadcast::error::RecvError, mpsc, RwLock},
    task::JoinSet,
};
...
pub enum Error {
    ...
    Timeout,
    #[error("Received a NACK for the Interest")]
    NackReceived,
    #[error("IO Error")]
...
pub struct App<S> {
    routes: BTreeMap<Name, Box<dyn InterestCallback>>,
...
struct InterestToSend<T> {
    interest: Interest<T>,
    sign: bool,
    notifier: sync::oneshot::Sender<Name>,
}
    ...
    S: SignMethod + Send + Sync + 'static,
{
    ...
    pub fn route<F>(&mut self, name: Name, func: F) -> &mut Self
    where
        F: InterestCallback + 'static,
    {
        self.routes.insert(name, Box::new(func));
        self
    }
